...
import { BaseReasonModel, ReasonModel } from '@/Models/Reason';
...
export default function Index(props: Props) {
  ...
  const [pagination, setPagination] = React.useState<MRT_PaginationState>({
    pageIndex: reasons.current_page - 1,
    pageSize: reasons.per_page,
  });
  ...
  const dataColumns = React.useMemo<MRT_ColumnDef<ReasonModel>[]>(
    () => [
      {
        header: 'No',
        accessorFn(row) {
    ...
    ], []) as MRT_ColumnDef<ReasonModel>[]

  const table = useMaterialReactTable({
    columns: dataColumns,
    data: reasons.data,
    rowCount: reasons.total,
    ...
    renderRowActions: ({ row }) => (
      <div className="flex items-center justify-center  gap-2">
        <MuiInertiaLinkButton
          color="primary"
          href={route('reason.show', row.original.id)}
        >
          Show
        </MuiInertiaLinkButton>
      </div>
  ...
  return (
    <AdminTableLayout
      title="Daftar Penyebab Masalah"
      addRoute={route('reason.create')}  
      addRouteTitle='Tambah Penyebab Masalah'
    >
