...
export default function Index(props: Props) {
  const sub_diseases = props.sub_diseases;
  ...
  const [pagination, setPagination] = React.useState<MRT_PaginationState>({
    pageIndex: sub_diseases.current_page - 1,
    pageSize: sub_diseases.per_page,
  });
  ...
  const dataColumns = React.useMemo<MRT_ColumnDef<SubDiseaseModel>[]>(
    () => [
      {
        header: 'No',
        accessorFn(row) {
    ...
    ], []) as MRT_ColumnDef<SubDiseaseModel>[]

  const table = useMaterialReactTable({
    columns: dataColumns,
    data: sub_diseases.data,
    rowCount: sub_diseases.total,
    ...
    renderRowActions: ({ row }) => (
      <div className="flex items-center justify-center  gap-2">
        <MuiInertiaLinkButton
          color="primary"
          href={route('sub-disease.show', row.original.id)}
        >
          Show
        </MuiInertiaLinkButton>
      </div>
    ...
    renderDetailPanel: ({ row }) => (
      <div className="flex flex-col gap-2">
        <div className="flex flex-col gap-1">
          ...
          <span>{row.original.disease.diagnosis}</span>
        </div>
      </div>
    )
  });
  ...
  return (
    <AdminTableLayout
      title="Daftar Sub Penyakit"
      addRoute={route('sub-disease.create')}
      addRouteTitle='Tambah Sub Penyakit'
    >
